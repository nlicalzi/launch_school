function getSelectedColumns(numbers, cols) {
  ...
  for (var i = 0, length = numbers.length; i < length; i += 1) {  // variables declared without let, meaning
    for (var j = 0, length = cols.length; j < length; j += 1) {   // here we overwrite the length variable
      if (!result[j]) {
        result[j] = [];
      }
